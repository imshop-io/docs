...
* **`interlinksParam`** - название параметра по которому происходит линковка (цвет, памят, жесткость) (ранее `interlinkingParam`)
  ...
  * **`type`** - тип перелинковки. опционально. **для стандартной перелинковки передавать не требуется**. Поддерживаются следующие типы:
    * **`unavailable`** - информирует покупателя, что товар не доступен в этом варианте. нажатие на эту кнопку приведет к сбросу / смене других параметров перелинковки на доступные. отображается в виде кнопки с пунктирной границей/обводкой. Под кнопками перелинковки покупатель увидит пояснение, что обозначает пунктирная обводка кнопки
