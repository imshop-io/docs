...
* `paymentMethodId` - выбранный покупателем способ оплаты. Берется из ответа [webhook получения способов оплат](payments.md)
* `paymentId` - Идентификатор платежа
...
* `paymentId` - идентификатор платежа
* `success` - всегда `true`
* `paymentCaptured` - `true` усли платеж успешно проведен, `false` если платеж не прошел
