...
Если в запросе присутствуют некорректные данные — их всегда можно уточнить на этапе подтверждения заказа по телефону.
...
* **`orders`** — список заказов \(за 1 запрос может выгружаться более 1 заказа\)
  ...
  * **`addressData`** — подробные данные об адресе доставки, включая идентификаторы из КЛАДР и ФИАС, объект «[Местоположение](../obekt-mestopolozhenie.md)»
