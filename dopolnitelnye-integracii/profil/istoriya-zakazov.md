...
{% hint style="info" %}
Обратите внимание на параметр **nextPage.** Это номер следующей страницы истории заказов. **nextPage** должен быть целым числом. Если параметр **nextPage** отсутствует или не является числом, то запрос на получение следующей страницы выполнен не будет.&#x20;
{% endhint %}
