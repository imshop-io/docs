...
* **`availablePromocodes`** – доступные промокоды (в разработке)
  * **`promocode`** – промокод
  * **`title`** – заголовок
  * **`description`** – описание (пользователь видит до 20 символов; остальной текст доступен по нажатию на информер рядом с заголовком)
  * **`availableUntil`** – крайний срок возможности активации промокода (отображается по нажатию на информер)
* **`extraServices` -** дополнительные услуги, [как реализовать \*клик](https://docs.imshop.io/dopolnitelnye-integracii/korzina-oformlenie-zakaza/dopolnitelnye-uslugi)
