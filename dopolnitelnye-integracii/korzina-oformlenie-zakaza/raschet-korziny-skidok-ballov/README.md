...
* **`availablePromocodes`** – доступные промокоды (в разработке)
  * **`promocode`** – промокод
  * **`title`** – заголовок
  * **`description`** – описание (до 20 символов, включая пробелы); напр. **"На заказ от 8000р"** или **"Только сегодня!"**
  * **`longDescription`** – подробное описание (текст доступен по нажатию на информер рядом с заголовком)
